...
class userModel
{
    ...
    // Hàm xử lý Đăng nhập
    public function handleLogin($username,$phone, $password)
    {
        $stmt = $this->pdo->prepare("SELECT * FROM users WHERE username=? OR phone=?  AND password=?");
        $stmt->execute([$username,$phone, $password]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }
    public function handleLoginByUsername($username,$password){
        $stmt = $this->pdo->prepare("SELECT * FROM users WHERE username=?  AND password=?");
        $stmt->execute([$username, $password]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }
    public function handleLoginByPhone($phone,$password){
        $stmt = $this->pdo->prepare("SELECT * FROM users WHERE phone=?  AND password=?");
